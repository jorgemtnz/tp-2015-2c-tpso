...
void cargaProcesoaCPU(char* dirArchivo, t_mCod* mCodCPU) { //solo hay un CPU en este hilo por lo que se crea en el hilo  mCodCPU
...
//++++++++++++++++++++++++++++++++
	...
	while ((cpu->mCodCPU->bufferInstrucciones[i] != NULL) && (i != cpu->pcbPlanificador->tamanioRafaga)) {
		estructura = ejecuta_Instruccion(cpu->mCodCPU->bufferInstrucciones[i], cpu);
